...
/*jshint browser: true*/
"use strict";
let reason_for_kick = ''; // adding reason to the disconnected tab for when you get kicked.
...
const socketInit = (() => {
  ...
  return port => {
    ...
    socket.onmessage = function socketMessage(message) {
      ...
      break;
            case 'y': {
              reason_for_kick = m[0]; // add the reason retrieved from server.js
                console.log(reason_for_kick)
      }
      break;
...
const gameDrawDisconnected = (() => {
  ...
  return () => {
    ...
    console.log(`[REASON]: ${reason_for_kick}`)
    if(reason_for_kick !== '' || reason_for_kick !== null){
    text.disconnected.draw(reason_for_kick, global.screenWidth / 2, global.screenHeight / 2, 30, color.grey, 'center');
    } else {
      text.disconnected.draw('You got disconnected :(', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiWhite, 'center');
    }
